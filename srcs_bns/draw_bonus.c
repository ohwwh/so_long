...
void	map_draw_m(t_vars *vars, int x, int y)
{
	...
	while (j < y + 2)
	{
		...
		while (i < x + 2)
		{
			if (vars->map[loc(i, j, vars)] != '1')
			{
				ft_img(vars, vars->tile00, i * 64, j * 64);
				if (vars->map[loc(i, j, vars)] == 'C')
					ft_img(vars, vars->collect, i * 64, j * 64);
				else if (vars->map[loc(i, j, vars)] == 'E')
...
void	map_draw_2(t_vars *vars, int i, int j, int *n)
{
	while (i < vars->w)
	{
		...
		{
			ft_img(vars, vars->tile00, i * 64, j * 64);
			if (vars->map[loc(i, j, vars)] == 'P')
			{
				vars->objs[0]->x = i * 64;
				vars->objs[0]->y = (j - 1) * 64;
			}
			else if (vars->map[loc(i, j, vars)] == 'R' && vars->game_level != 3)
			{
				vars->objs[*n]->x = i * 64;
